...
export class AuthController {
  ...
  @ApiOperation({ summary: 'Создание аккаунта' , description:"Создает новый аккаунт пользователя"})
  @ApiOkResponse({type:AuthResponse})
  @ApiBadRequestResponse({description:"Некорректные входные данных"}) // это при валидации выбросит
  @ApiConflictResponse({description:'Пользователь с такой почтой уже существует'}) // это в Сервисе выбросит throw new ConflictException('Пользователь с такой почтой уже существует')
  @Post('register')
  ...
  @ApiOperation({ summary: 'Вход в систему' , description:"Авторизует пользователя и выдает токен доступа"})
  @ApiOkResponse({type:AuthResponse})
  @ApiBadRequestResponse({description:"Некорректные входные данных"}) // это при валидации выбросит
  ...
  @ApiOperation({ summary: 'Обновление токена' , description:"Генерирует новый токен доступа"})
  @ApiOkResponse({type:AuthResponse})
